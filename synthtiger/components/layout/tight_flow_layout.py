...
class TightFlowLayout(Component):
    def __init__(
        self,
        length=None,
        space=(0, 0),
    ...
    def apply(self, layers, meta=None):
        ...
        if vertical:
            for idx, layer in enumerate(layers):
                # layer.bbox gives [rel_x, rel_y, width, height]
                w, h = layer.bbox[2], layer.bbox[3]

                if idx > 0 and length is not None and (y + h) > length:
                    y = 0
                    x = line
                    groups.append([])

                # Safely translate the layer pixels using the topleft tuple
                layer.topleft = (x - layer.bbox[0], y - layer.bbox[1])

                y += h + space
                line = max(x + w + line_space, line)
                groups[-1].append(layer)
        else:
            for idx, layer in enumerate(layers):
                w, h = layer.bbox[2], layer.bbox[3]

                if idx > 0 and length is not None and (x + w) > length:
                    x = 0
                    y = line
                    groups.append([])
